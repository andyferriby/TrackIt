...
class TagsCell: UITableViewCell {
    ...
    @objc weak var delegate: TagsCellDelegate?
}

extension TagsCell: EntryConfigurable {
    @objc func configureWithEntry(_ entry: Entry) {
        guard let tags = entry.tags?.allObjects as? [Tag] else { return }
...
extension TagsCell: TagConfigurable {
    @objc func configureWithTags(_ tags: [Tag]) {
        tagListView.removeAllTags()
        tagListView.delegate = self
        for tag in tags {
            let tagView = tagListView.addTag(tag.name!)
...
extension TagsCell: TagListViewDelegate {
    ...
    @objc func tagRemoveButtonPressed(_ title: String, tagView: TagView, sender: TagListView) {
